...
// Then we iterate through the sorted logs which are total M. At each iteration, we invoke the union(a, b) function which tooks a(M) Time. so the time complexity is O(M*a(M)), where a is the Inverse Ackermann Function and N will never go higher than 4 in the universe.
